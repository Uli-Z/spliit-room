import { headerAliasesByLanguage } from '@/lib/splitwise-mapping-data'
import {
  SplitwiseColumn,
  SplitwiseExportLanguage,
} from '@/lib/splitwise-types'

export type { SplitwiseColumn, SplitwiseExportLanguage } from '@/lib/splitwise-types'
...
export function detectSplitwiseHeaders(
  ...
  const detectionLanguages = (Object.keys(headerAliasesByLanguage) as SplitwiseExportLanguage[]).filter(
  ...
  for (const language of detectionLanguages) {
    ...
    const missing = headerFields.filter(
      (field) => matches[field] === undefined,
    )
